...
import { ActivityIndicator, Colors,TextInput,Button,Portal,Modal,Provider} from 'react-native-paper';
...
export default function Test({route,navigation}) {
  ...
  const {DataReducer:data,QuesidReducer:quesid,QuesarrayReducer:quesarr,ResultReducer:result,SectionReducer:section,ModalReducer:modal} = useSelector(state => state)
  ...
  return (load?
    ...
    <View style={classes.container}>
      ...
      <View style={classes.mainView}>
          <View style={classes.mainViewleft}>
              <View style={classes.sec1}>
                  {data.section_id.map((sec,index)=>{
                      return (
                          <TouchableOpacity style={{padding:5}} key={index} onPress={()=>{secChange(sec)}}>  
                              <Text style={{backgroundColor:'#38A9EB',color:'white',padding:8,margin:10,borderRadius:10}} >
                              {sec.title}
                              </Text>
                          </TouchableOpacity>
                      )
              ...
              <View style={classes.sec3}>
                  <View>
                      <Text  style={{backgroundColor:'#38A9EB',color:'white',padding:8}} >
                      {section.title}
                      </Text>
                  </View>
              </View>
          ...
          <Provider>
                <Portal>
                    <Modal visible={modal} onDismiss={()=>{setModal(false)}} contentContainerStyle={classes.modal}>
                        <View style={classes.paper}>
                            <Text style={{fontSize:20}} >
                                Do You Wanna Submit?
                            </Text>
                            <View style={{display:'flex',flexDirection:'row',paddingVertical:30}}>
                                <Button mode="contained" style={{backgroundColor:'#449D44'}} onPress={()=>submitTest()}>
                                    Submit
                                </Button>
                                <Button mode="contained" style={{backgroundColor:'#E74500'}} onPress={()=>setModal(false)}>
                                    Cancel
                                </Button>
                            </View>
                        </View>
